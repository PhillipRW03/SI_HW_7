...
def make_positions_table(data, cur, conn):
    ...
    for i in range(len(positions)):
        cur.execute(
            "INSERT OR IGNORE INTO Positions (id, position) VALUES (?,?)", (i, positions[i]))
    conn.commit()
...
def make_players_table(data, cur, conn):
    ...
    for player in data.get("squad"):
        ...
        posid = cur.execute(
            "SELECT id FROM Positions WHERE Positions.position = ?", [pos])
        posid = posid.fetchone()[0]
        cur.execute("INSERT OR IGNORE INTO Players (id, name, position_id, birthyear, nationality) VALUES (?,?,?,?,?)", (player.get(
            "id"), player.get("name"), posid, player.get("dateOfBirth").split('-')[0], player.get("nationality")))
    conn.commit()
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_type_speed_defense_search(self):
        b = sorted(position_birth_search(
            'Goalkeeper', 35, self.cur, self.conn))
